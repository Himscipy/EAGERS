function Profile = loadSched2(Data,Date)
...
Profile = zeros(nS,1);
[newDay,D_of_Y, D_of_M] = dayOfYear(Date);
...
for d = 1:1:days
    ...
    if Date(p)>=floor(Data.Timestamp(1)) && (Date(p2)-dt)<floor(Data.Timestamp(end))
        ...
        [data_newDay,data_D_of_Y,data_D_of_M] = dayOfYear(Data.Timestamp);
...
end%Ends function loadSched2

function [newDay,D_of_Y, D_of_M] = dayOfYear(Date)
D = datevec(Date(1));
