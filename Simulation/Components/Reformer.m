function Out = Reformer(varargin)
...
elseif length(varargin)==2 %% Have inlets connected, re-initialize
    ...
    %     K_WGS = exp(4189.8./ReformerOut.T -3.8242);% Water gas shift equilibrium constant
        ...
        tC1 = (block.Vol_1*Cp_cold*PprimaryIn./(block.Ru*Primary.Outlet.T));
        ...
        if isfield(Inlet,'Flow2')
            tC3 = (block.Vol_2*Cp_hot*PsecondaryIn./(block.Ru*SecondaryOut.T));
            ...
            dY(end) = (Flow2-NsecondaryOut)*block.Ru*Inlet.Flow2.T/block.Vol_2;
        end

        dY(end-s) = (Flow1-NprimaryOut)*block.Ru*Inlet.Flow1.T/block.Vol_1;
        Out = dY;
    end
end
end%Ends function Reformer
...
block.UpperBound = inf*ones(nS,1);
block.LowerBound = zeros(nS,1);

block.tC = ones(nS,1); % time constant for derivative dY

block.Scale(1:nodes,1) = block.Tstates(1:nodes);%temperature (K)
Cp = SpecHeat(Primary);
block.tC(1:block.nodes,1) = block.Vol_1*Cp*block.ReformedPin.IC./(block.Ru*block.Tstates(1:nodes));
n = N*nodes;
...
for i = 1:1:length(block.spec1)
    ...
    block.tC(n+1:n+nodes,1) = (block.Vol_1*block.ReformedPin.IC)./(block.Tstates(1:nodes)*block.Ru);
    n = n + nodes;
end
...
if block.hotStream ==1
    ...
    block.tC(1+2*nodes:3*nodes,1) = block.Vol_2*Cp*block.CooledPin.IC./(block.Ru*block.Tstates(1:nodes));
    ...
    for i = 1:1:length(block.spec2)
        ...
        block.tC(n+1:n+nodes,1) = (block.Vol_2*block.CooledPin.IC)./(block.Tstates(2*nodes+1:3*nodes)*block.Ru);
        n = n + nodes; 
    end
end
...
end%Ends function SolveDynamic