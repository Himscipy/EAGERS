...
%%%---%%% end of user defined variables    
    ...
    FuelFlow  = sum(Current)/(2*block.F*1000)/(block.FuelUtilization*(4*block.Fuel.CH4+block.Fuel.CO+block.Fuel.H2)); % fuel flow rate,  current/(2*F*1000) = kmol H2
    block.Recirc.Anode = anodeRecircHumidification(block.Fuel,FuelFlow,0.7,block.Steam2Carbon,sum(Current)/(2*block.F*1000),0.5);
    ...
    block.FuelFlowInit  = sum(Current)/(2*block.F*1000)/(block.FuelUtilization*(4*block.Fuel.CH4+block.Fuel.CO+block.Fuel.H2))*block.Cells; % fuel flow rate,  current/(2*F*1000) = kmol H2
...
%     end
    ...
    block.AirFlow = block.Cells*sum(Current)/(4*block.F*block.Flow2Spec.O2)/1000/block.OxidantUtilization;%kmol of oxidant
...
elseif length(varargin)==2 %% Have inlets connected, re-initialize
    ...
    for j = 1:1:length(block.Flow2Dir(1,:));
        ...
        if block.ClosedCathode %closed end cathode
            Flow2.Outlet.O2(k,1) = Flow2.Inlet.O2(k,1) - nCurrent(k)/(4*block.F*1000); 
            if any(strcmp(block.FCtype,{'MCFC','MCEC'}))
                Flow2.Outlet.CO2(k,1) = Flow2.Inlet.CO2(k,1) - nCurrent(k)/(2*block.F*1000); 
            end
        end
    end
    ...
    if strcmp(string1,'Outlet')
        ...
        Tags.(block.name).SinglePassUtilization = Tags.(block.name).Current*block.Cells/(2000*block.F)/(4*Inlet.Flow1.CH4+Inlet.Flow1.CO + Inlet.Flow1.H2);
        ...
        Tags.(block.name).Efficiency = Power/(NetFlow(Inlet.Flow1)*HeatingValue(Inlet.Flow1));
    ...
    elseif strcmp(string1,'dY')
        ...
        for i=1:1:length(block.Flow1Dir(1,:))
            k = block.Flow1Dir(:,i);
            dY(3*nodes+k)= (QT(3*nodes+k) + HinAnode(k) - HoutAnode(k) + Qion(k) - nPower(k) - Qgen(k))./block.tC(3*nodes+k);  %Anode
            if i>1
        ...
        for i = 1:1:length(block.Spec2)
            if strcmp(block.Spec2{i},'CO2') && any(strcmp(block.FCtype,{'MCFC';'MCEC'}))
                dY(n+1:n+nodes)= (Flow2.Inlet.CO2 - Y(n+1:n+nodes) - nCurrent/(2*block.F*1000))./block.tC(n+1:n+nodes);  %CO2 species concentration with CO2 crossover
            elseif strcmp(block.Spec2{i},'O2') && any(strcmp(block.FCtype,{'SOFC';'SOEC';'MCFC';'MCEC'}))
                dY(n+1:n+nodes)= (Flow2.Inlet.O2 - Y(n+1:n+nodes) - nCurrent/(4*block.F*1000))./block.tC(n+1:n+nodes);%O2 species concentration with O2 crossover
            else
        ...
        dY(n+1) = (NetFlow(Inlet.Flow1)-Nflow1)*block.Ru*Inlet.Flow1.T/block.tC(n+2);
        if block.ClosedCathode %closed end cathode
            dY(n+2) = 0;%(NetFlow(Inlet.Flow2)-sum(nCurrent)/(4*F*1000))*block.Ru*Inlet.Flow2.T/block.tC(n+1); %no flow out, so anything not used in electrochemistry adds to pressure
        else
            Nflow2 = block.Pfactor2*max(0.1,(P_flow2-Inlet.Flow2Pout));%total cathode flow out
            dY(n+2) = (NetFlow(Inlet.Flow2)-Nflow2)*block.Ru*Inlet.Flow2.T/block.tC(n+1);%working with total flow rates so must multiply by nodes & cells
        end
        Out = dY;
    end
end
...
%     SinglePassUtilization = (sum(block.Current)*block.Cells/(2000*F))/(4*Inlet.Mixed.CH4+Inlet.Mixed.CO + Inlet.Mixed.H2);
    ...
    if block.ClosedCathode % Ensure there is enough oxidant so Flow2 does not go negative
        Inlet.Flow2.O2 = block.Cells*TotCurrent/(4000*block.F*block.Flow2Spec.O2)/block.OxidantUtilization;%kmol of oxidant
        if any(strcmp(block.FCtype,{'MCFC';'MCEC'}))
            Inlet.Flow2.CO2 = block.Cells*TotCurrent/(2000*block.F*block.Flow2Spec.O2)/block.OxidantUtilization;%kmol of oxidant
        end
    end
    if firstSolve ==1 %solving block to convergence without other blocks or controller
        ...
        if strcmp(block.CoolingStream,'cathode')%air flow balances heat generation to maintain deltaT, heat transfer to anode and any fuel reforming is accounted for
            block.FuelFlowInit  = TotCurrent/(2*block.F*1000)/(block.FuelUtilization*(4*block.Fuel.CH4+block.Fuel.CO+block.Fuel.H2))*block.Cells; % Fresh fuel flow rate,  current/(2*F*1000) = kmol H2
        ...
        elseif strcmp(block.CoolingStream,'anode') %oxidant flow rate determined by current, fuel flow rate is now determined by thermal balancing
            block.AirFlow = block.Cells*TotCurrent/(4000*block.F*block.Flow2Spec.O2)/block.OxidantUtilization;%kmol of oxidant
            ...
            block.FuelFlowInit  = block.Cells*TotCurrent/(2000*block.F)/(block.FuelUtilization*(4*block.Fuel.CH4+block.Fuel.CO+block.Fuel.H2)); % re-calculate with revised current
            block.FuelFlowInit = block.FuelFlowInit + ExtraFuel;
            block.FuelUtilization = block.Cells*TotCurrent/(2000*block.F)/(block.FuelFlowInit*(4*block.Fuel.CH4+block.Fuel.CO+block.Fuel.H2));
...
switch block.Reformer
    case 'internal'
        n = 6*block.nodes;
        block.tC(5*block.nodes+1:6*block.nodes) = (block.Vol_flow3*Cp.ref*block.Flow1_Pinit./(block.Ru*block.T.Flow3));
    case {'direct';'external';'adiabatic'}
        n = 5*block.nodes;
end
...
block.tC(1+block.nodes:2*block.nodes) = (block.Vol_flow2*Cp.cath*block.Flow2_Pinit./(block.Ru*block.T.Flow2));
block.tC(2*block.nodes+1:3*block.nodes) = (block.Vol_Elec*block.Density_Elec*block.C_Elec);
block.tC(3*block.nodes+1:4*block.nodes) = (block.Vol_flow1*Cp.an*block.Flow1_Pinit./(block.Ru*block.T.Flow1));
...
for i = 1:1:length(block.Spec2)
    block.tC(n+1:n+block.nodes) = (block.Vol_flow2*block.Flow2_Pinit)./(block.T.Flow2*block.Ru);  % cathode 
...
for i = 1:1:length(block.Spec1)
    ...
    block.tC(n+1:n+block.nodes) = (block.Vol_flow1*block.Flow1_Pinit)./(block.T.Flow1*block.Ru); %anode
...
switch block.Reformer
    case 'internal'
        for i = 1:1:length(block.Spec1)
            block.tC(n+1:n+block.nodes) = (block.Vol_flow3*block.Flow1_Pinit)./(block.T.Flow3*block.Ru); % reformer
...
if isfield(block,'tC')
    ...
    tC(1+nodes:2*nodes,1) = (block.Vol_flow2*Cp_2*block.Flow2_Pinit./(block.Ru*block.T.Flow2));
    tC(2*nodes+1:3*nodes,1) = (block.Vol_Elec*block.Density_Elec*block.C_Elec);
    tC(3*nodes+1:4*nodes,1) = (block.Vol_flow1*Cp_1*block.Flow1_Pinit./(block.Ru*block.T.Flow1));
    tC(4*nodes+1:5*nodes,1) = (block.Mass_plate1*block.C_plate1);
    if isfield(block,'Vol_flow3')
        Cp_3 = 42;% kJ/kmol*K
        tC(5*nodes+1:6*nodes,1) = (block.Vol_flow3*Cp_3*block.Flow1_Pinit./(block.Ru*block.T.Flow3));
    end
end
