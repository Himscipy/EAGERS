...
if length(varargin)==1 % first initialization
    ...
    switch block.FCtype
        case {'SOFC';'SOEC'}
            ...
            block.alpha=.7;
            block.ElecConst = 1.8e3; %(K/ohm*m) Electrolyte Constant  %default SOFC  = 9e7
...
%%%---%%% end of user defined variables    
    ...
    if strcmp(block.Specification,'power density')
        i_avg = -block.SpecificationValue/block.Voltage/1000; %convert mW/cm^2 to A/cm^2, assume an initial guess voltage of 1.3
    elseif strcmp(block.Specification,'current density')
...
%     nPower = Tags.(block.name).nVoltage'.*nCurrent/1000;
    ...
    if strcmp(string1,'Outlet')
        ...
        %% Tags
        Tags.(block.name).Flow1Outlet=Flow1.Outlet.T;
        Tags.(block.name).Flow2Outlet=Flow2.Outlet.T;
        ...
        %Tags.(block.name).Efficiency1 = (NetFlow(Flow1Out)*HeatingValue(Flow1Out))/Power;
        %Tags.(block.name).Efficiency1 = (NetFlow(Outlet.HX1.HotOut)*HeatingValue(Outlet.HX1.HotOut))/Power;
        [h,~] = enthalpy(block.TpenAvg,{'H2','H2O','O2'});
        h_rxn3 = h.H2+.5*h.O2-h.H2O;
        Vbalance = 1./(2*block.F).*h_rxn3; %voltage that balances heat
        Tags.(block.name).Efficiency2 = Vbalance/Voltage;
...
%         Voltage =  Tags.(block.name).Voltage;
    ...
    %             R.WGSref = Rref.WGS;
        ...
        for i=1:1:length(block.Flow1Dir(1,:)) %having the downstream nodes change temperature with the upstream nodes prevents propogation issues when taking larger time steps
            k = block.Flow1Dir(:,i);
            dY(nodes+k)= (QT(nodes+k) + Hin1(k) - Hout1(k) + Qion(k) - nPower(k) - Qgen(k))./block.tC(nodes+k); %Steam side %dY(nodes+k)= (QT(nodes+k) + Hin1(k) - Hout1(k) + Qion(k) - nPower(k))./block.tC(nodes+k);
             if i>1
        ...
        for i=1:1:length(block.Flow2Dir(1,:))
            k = block.Flow2Dir(:,i);
            dY(3*nodes+k)= (QT(3*nodes+k) + Hin2(k) - Hout2(k) - Qion(k))./block.tC(3*nodes+k);  %Air Side
            if i>1
...
block.tC(4*block.nodes+1:5*block.nodes) = (block.Mass_plate2*block.C_plate2);
...
if isfield(block,'tC')
    ...
    Cp_2 = SpecHeat(Flow2.Inlet);%     Cp_2 = 33;% kJ/kmol*K
    Cp_1 = SpecHeat(Flow1.Outlet);%     Cp_1 = 42;% kJ/kmol*K
...
dY(1+nodes:2*nodes)= (QT(1+nodes:2*nodes) + Hin1 - Hout1 + Qion - Power - Qgen)./tC(1+nodes:2*nodes); %Cathode
dY(1+2*nodes:3*nodes)= (QT(1+2*nodes:3*nodes) + Qgen)./tC(2*nodes+1:3*nodes); %Electrolyte Plate
dY(1+3*nodes:4*nodes)= (QT(1+3*nodes:4*nodes) + Hin2 - Hout2 - Qion)./tC(1+3*nodes:4*nodes);  %Anode
