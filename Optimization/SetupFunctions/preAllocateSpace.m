...
if strcmp(mode,'Design')
    ...
    Plant.Design.LBRelax = ones(NumSteps,1);%degree to which you relax the lower bound when doing cQP
    for j = 1:1:length(F)
...
elseif strcmp(mode,'Dispatch')
    ...
    Plant.Predicted.LBRelax = ones(NumSteps,1);%degree to which you relax the lower bound when doing cQP
    for j = 1:1:length(F)
        if isstruct(TestData.(F{j}))
            S = fieldnames(TestData.(F{j}));
            for i = 1:1:length(S)
