...
for i = 1:1:nG
    ...
    if ~isempty(Gen.states)
        ...
        else Organize.Out_vs_State{1,i} = ones(1,s);
        end
...
if any(strcmp('DistrictHeat',networkNames)) 
    if Plant.optimoptions.excessHeat == 1
        ...
        Organize.HeatVented = zeros(length(Plant.subNet.DistrictHeat.nodes),1);
    end
else
    Organize.HeatVented = [];
end
...
%%Special case of final state-of-charge equal to initial for comparrison to PNNL
if strcmp(Plant.Name,'PNNL_B')
    QP.Aeq(end+1,:) = zeros(1,length(QP.Aeq(1,:)));
    QP.beq(end+1,1) = 0;
    QP.Aeq(end,QP.organize{1,5}) = 1;
    QP.Aeq(end,QP.organize{end,5}) = -1;
end
    ...
    if isfield(Plant.Generator(i).QPform.output,'H')
        if ~isempty(strfind(Plant.Generator(i).Type,'Storage'))
            ...
            if isfield(Plant.Generator(i).QPform.output,'E')
                maxHeat = maxHeat + max(Plant.Generator(i).Size*Plant.Generator(i).Output.Capacity./Plant.Generator(i).Output.Electricity.*Plant.Generator(i).Output.Heat);
            elseif Plant.Generator(i).QPform.output.H(end)>0
                maxHeat = maxHeat + max(max(Plant.Generator(i).QPform.output.H))*Plant.Generator(i).Size;
            end
        end
    end
end
