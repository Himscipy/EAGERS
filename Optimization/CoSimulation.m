...
global Plant DispatchWaitbar DateSim Last24hour TestData CurrentState
...
CurrentState.Buildings = zeros(2,1) + 22.22;
DateSim = Date;
nS = 365*24/Plant.optimoptions.Resolution+1;
TestData.RealTimeData.Timestamp = linspace(datenum([2017,1,1]),datenum([2018,1,1]),nS)';
...
if TestData.RealTimeData.Timestamp(1)<(DateSim-1) && TestData.RealTimeData.Timestamp(end)>=DateSim
...
while Si<NumSteps-1
    ...
    [Dispatch,OptSchedule] = NRELoptimization2(CurrentState.Generators,CurrentState.Buildings(1,:),Forecast,scaleCost);
    %% record predictions
    for i = 1:1:length(dems)
    ...
    CurrentState.Buildings = zeros(2,1) + 22;
