...
if strcmp(Gen.Type,'Solar') 
    ...
    if strcmp(str,'Actual') && isfield(Plant.Weather,'Irrad') % Actual ...
    ...
    D = datevec(Date(1));
    h_of_y = mod(24*(Date - datenum([D(1),1,1])),8760); % Hour since start ...
        % of year
    DirNormal = interp1(hofY_Irrad,[Irrad(1);Irrad],h_of_y);
    [~,~,Azimuth,Zenith] = SolarCalc(Location.Longitude,Location.Latitude, ...
        Location.TimeZone,Date);
    if strcmp(Gen.Tracking,'fixed')
        % The data for the renewable power is recorded in 15 min intervals
        % starting at Jan 1st
        Power = Gen.Sizem2 * (DirNormal/1000) .* cosd(Zenith-Gen.Tilt) .* ...
            (cosd(Azimuth-Gen.Azimuth)) * Gen.Eff;
