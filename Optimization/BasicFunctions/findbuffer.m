        ...
        for j = 1:1:length(equip)
            if ismember(Plant.Generator(equip(j)).Type,include)
                if strcmp(out,'H') && strcmp(Plant.Generator(equip(j)).Type,'CHP Generator')
                    chargeCapacity = chargeCapacity+Plant.Generator(i).Size*Plant.Generator(i).QPform.output.H/Plant.Generator(i).QPform.output.E; %CHP generators heat ratio
                else chargeCapacity = chargeCapacity+Plant.Generator(i).Size;
                end
            end
        end
        for j = 1:1:length(equip)
