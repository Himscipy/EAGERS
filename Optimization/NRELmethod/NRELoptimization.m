function GenDisp = NRELoptimization(Forecast,scaleCost)
...
Forecast.Demand.H = 1e4*ones(nS,1)*2; % incidental heating load
Forecast.Demand.H(1:nS/4) = 0.5e4*2;
Forecast.Demand.H(nS*3/4:end) = 0.5e4*2;
...
Ebat_max = Plant.Generator(5).Size*(Plant.Generator(5).VariableStruct.MaxDOD/100); % usable battery capacity (J)
...
for i = 1:nS
    ...
    T_new = Ti + dt*1/Cr*(1/R*(Twi-Ti) + mi*Cp*(Ts-T_approx) + Forecast.Demand.H(1));
...
cvx_solver_settings( 'NumericFocus', '3' ) 
    ...
    for i = 1:nS
        ...
        Pfi = alpha_f*mi; % ???
        Pei = Pchei + Pfi + Forecast.Demand.E(i)*1000 + Pbat_charge_i - Pfcfi*eta_fce - Pbat_discharge_i;
        J1(i,1) = scaleCost(i,3)*Pei/1000 + scaleCost(i,4)*Pfcfi*eta_fce - cas(i)*R_asi;
...
for i = 1:nS
    ...
    mpi(i,1) = x((i-1)*6+3); %mass flow of HVAC in kg/s
    Pbat_discharge(i,1) = u((hour-1)*4+3);
    Pbat_charge(i,1) = u((hour-1)*4+4);
    
    GenDisp(i,3) = x((i-1)*6+5)*eta_fce/1000; %fuel cell electrical output
    GenDisp(i,4) = x((i-1)*6+4)/1000;%chiller output in kW of cooling
    GenDisp(i,5) = x((i-1)*6+6)/3600/1000;%battery state of charge in kWh
    GenDisp(i,1) = GenDisp(i,4)/COP + alpha_f*mpi(i,1)/1000 + Forecast.Demand.E(i) + Pbat_charge/1000 - GenDisp(i,3) - Pbat_discharge(i,1)/1000; %power to/from the electric grid in kW
    R_as_vec(i,1) = R_as(hour); %ancillary service  
end