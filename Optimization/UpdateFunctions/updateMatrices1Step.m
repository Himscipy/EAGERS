...
if ~isempty(FirstProfile)
    for i = 1:1:nG
        ...
        elseif any(strcmp(Plant.Generator(i).Type,{'Electric Storage';'Thermal Storage';'Hydro Storage';}))
            ...
            QP.ub(s) = min(QP.ub(s), FirstProfile(t+1,i)/dt(t));%you can't discharge more than you have stored            
        ...
        if strcmp(Plant.Generator(i).Type,{'Hydro Storage';})
            ...
            QP.beq(QP.Organize.HydroEqualities(i)) = -FirstProfile(i)*Plant.Generator(i).QPform.Stor.Power2Flow;
        end
...
if ismember('Hydro',networkNames) && ~isempty(Plant.subNet.Electrical.lineNames)
    ...
    for i = 1:1:nG
        if strcmp(Plant.Generator(i).Type,{'Hydro Storage';}) && isfield(Plant.Generator(i).QPform,'S') 
            states = QP.Organize.States{i};
            QP.f(states(2)) = 0.0001/Plant.Generator(i).QPform.Stor.Power2Flow;
        end
    end
end 
...
for i = 1:1:nG
    if ~isempty(QP.Organize.States{i})
        ...
        elseif ismember(Plant.Generator(i).Type,{'Electric Storage';'Thermal Storage';'Hydro Storage';})%all  storage
            ...
            if isempty(FirstProfile)
                QP.f(states(1)) = marginal.(S{1});
                if any(strcmp(networkNames,'Electrical')) && strcmp(S{1},'H') % first initialization give arbitrarily high cost to storage (but not thermal storage if in conjunction with electric dispatch)
                    H(states(1)) = 0;
                elseif strcmp(S{1},'W') %Hydro needs to have high cost, but not too high that it never uses it; set arbitrary number
                    H(states(1)) = 1e5*marginal.(S{1});
                else 
                    H(states(1)) = 1e8*marginal.(S{1});
                end
            else
